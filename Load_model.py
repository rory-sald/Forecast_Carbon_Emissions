...
def prediction_plot(UK_ES, test_predict):
	...
	plt.title('LSTM Predictions',fontsize=16, pad=2)
	plt.grid(True)
	plt.show()
...
train, test = df1[:train_size,:],df1[train_size:len(df1),:]
#convert dataset into right shape in order to input into the DNN
trainX, trainY = convert2matrix(train, i)
testX, testY = convert2matrix(test, i)


model = load_model('/Users/rems/Library/CloudStorage/OneDrive-UniversityofBath/IMEE_FYP/Code/my_model(6, 20, 20, 1, 10, \'ANN\')')
future_predictions = []

x_input = np.reshape(np.concatenate([trainX[-1][-i:], [trainY[-1]]])[1:],(1,i))
for m in range(9):
