...
try:
	best_MAPE = np.load("LSTM_best_MAPE")
except:
	best_MAPE = 10
MAPEA = []
for p in n_lookback:
		...
		for j in n_nodes:
			for k in n_epochs:
				for l in n_batch:
					...
					model=Sequential()
					model.add(LSTM(i, activation='relu', input_shape=(i, 1), return_sequences=False))
					...
					if MAPE <= best_MAPE:
						best_MAPE = MAPE
						best_model = model
						best_params = (i,j,k,l,p,"LSTM")

best_model.save(f'my_model{best_params}')
np.save("LSTM_best_MAPE", best_MAPE)
