...
def prediction_plot(UK_ES, test_predict):
	...
	plt.title('CNN Predictions',fontsize=16, pad=2)
	plt.grid(True)
	plt.show()
		...
		for j in n_nodes:
			for k in n_epochs:
				for l in n_batch:
					...
					model=Sequential()
					model.add(Conv1D(filters = i, activation='relu', kernel_size = 3, input_shape=(i, 1), padding = 'same'))
					...
					future_predictions = []
					x_input = np.reshape(np.concatenate([trainX[-1], [trainY[-1][0]]])[1:],(1,i))
					#x_input = trainX[-1]
					...
					if MAPE <= best_MAPE:
						best_MAPE = MAPE
						best_model = model
						best_params = (i,j,k,l,p,"CNN")

best_model.save(f'my_model{best_params}')
np.save("CNN_best_MAPE", best_MAPE)
print(f'my_model{best_params}',"best")
MAPEA = sorted(MAPEA)
print(MAPEA)
				
model = load_model(f'/Users/rems/Library/CloudStorage/OneDrive-UniversityofBath/IMEE_FYP/Code/my_model{best_params}')
future_predictions = []
input_size = best_params[0]
x_input = np.reshape(np.concatenate([trainX[-1][-input_size:], [trainY[-1][0]]])[1:],(1,input_size))
for m in range(9):
